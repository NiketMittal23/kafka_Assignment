...
public class Consumer {
    ...
    public static void consumer() throws IOException {
        ...
        properties.put("value.deserializer", "com.kafka_assignment.serializer.UserDeserializer");
        ...
        try (KafkaConsumer<String, input> kafkaConsumer = new KafkaConsumer(properties)) {
            kafkaConsumer.subscribe(topics);
            while (true) {
                ConsumerRecords<String, input> messages = kafkaConsumer.poll(1000);
                for (ConsumerRecord<String, input> message : messages) {
